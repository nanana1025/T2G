...
class SimTSCTrainer:
	...
	def fit(self, model, X, y, train_idx, distances, lc, K, alpha, test_idx=None, report_test=False, batch_size=128, epochs=500):
		self.K = K
		self.alpha = alpha
...
class SimTSC(nn.Module):
	def __init__(self, input_size, nb_classes, lc, num_layers=1, n_feature_maps=64, dropout=0.5):
	...
	def forward(self, x, adj, K, alpha):
		...
		for i in range(len(adj)):
			...
			for j in ranks[i][:K]:
				...
				_sparse_value.append((1/np.exp(alpha*adj[i][j]*self.lc[i][j])))
				#_sparse_value.append((1/np.exp(alpha*adj[i][j]))*1)
