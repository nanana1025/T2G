...
class SimTSCTrainer:
	...
	def fit(self, model, model_sliding,model_sliding2, X, y, train_idx, distances, K, alpha, test_idx=None, report_test=False, batch_size=128, epochs=500):
		...
		#print("Train idx length:", len(train_idx))
...
class GCNEmbedding2(nn.Module):
	def __init__(self, normalized_data_list, sliding_adj, nb_classes, num_layers=1, n_feature_maps=64, dropout=0.5):
		...
		self.dropout = dropout  # dropout 값을 한 번만 정의
		
		# ResNetBlock을 동적으로 초기화하기 위해 나중에 설정
		self.block_1 = None
		self.block_2 = None
		self.block_3 = None
		self.block_4 = None
	...
	def forward(self, idx):
		device = next(self.parameters()).device  # 모델이 올라간 디바이스 확인
		graph_embeddings = []  # embedding 결과 저장 리스트

		for i in idx:
			x = self.slidng_data[i].to(device)  # 입력 데이터 가져오기
			adj = self.sliding_adj[i].to(device)

			# input_size를 자동 감지하여 ResNet 블록 초기화 (최초 한 번만)
			...
			# GCN 레이어 적용
			for layer in self.gc_layers[:-1]:  # 마지막 레이어 제외
				x = F.relu(layer(x, adj))
				x = F.dropout(x, self.dropout, training=self.training)

			x = self.gc_layers[-1](x, adj)  # 마지막 GCN 레이어
			graph_embeddings.append(x.mean(dim=0))  # 그래프 임베딩 저장

		# torch.stack()을 사용하여 리스트 -> 텐서 변환
...
class GCNEmbedding(nn.Module):
	def __init__(self, nb_classes, num_layers=1, n_feature_maps=64, dropout=0.5):
		...
		self.block1_total_time = 0

		# 레이어 초기화는 forward()에서 input_size 기반으로 진행
	...
	def initialize_blocks(self, input_size):
		""" input_size 기반으로 ResNetBlock 동적 생성 및 GPU 이동 """
		device = next(self.parameters()).device  # 현재 모델의 장치 확인
	...
	def forward(self, x, adj):
		...
		self.block1_total_time += end - start
		# x = self.block_2(x)
		# x = self.block_3(x)
		# x = self.block_4(x)
		x = F.avg_pool1d(x, x.shape[-1]).squeeze()
...
class SimTSC(nn.Module):
	...
	def forward(self, gcn_embedding, idx, x, adj, K, alpha):
		...
		'''
		#len(self.slidng_data)
		#print(f"len(self.slidng_data) = {len(self.slidng_data)}")
		...
		# ✅ 이동이 잘 되었는지 확인
		#for name, param in gcn_embedding.named_parameters():
		#	print(f"{name} is on {param.device}")

		for i in idx:
			#i = 67  # 디버깅을 위해 강제 설정

			# ✅ NumPy 배열을 PyTorch Tensor로 변환 후 GPU로 이동
			sx = torch.from_numpy(np.array(self.slidng_data[i])).float().to(device)
			sx = sx.permute(0, 2, 1)  # ✅ 기존 형식과 동일하게 변경

			# ✅ Sparse Tensor 유지 & GPU로 이동
			sadj = self.sliding_adj[i]
			if sadj.is_sparse:
			...
			'''
			print(f"i = {i}")
			print(f"type(sx) = {type(sx)}, type(sadj) = {type(sadj)}")
			print(f"sx.shape = {sx.shape}, sadj.shape = {sadj.shape}")
			print(f"sx device = {sx.device}, sadj device = {sadj.device}, model device = {next(gcn_embedding.parameters()).device}")
			'''
			y[num] = gcn_embedding(sx, sadj)
			num += 1
