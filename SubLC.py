...
def train(X, y, train_idx, test_idx, distances, lc,device, logger, K, alpha, batch_size, epoches):
	...
	model = SimTSC(input_size, nb_classes, lc)
	model = model.to(device)
	trainer = SimTSCTrainer(device, logger)

	model = trainer.fit(model, X, y, train_idx, distances, lc, K, alpha, None, False, batch_size, epoches)
...
if __name__ == "__main__":
	...
	lc_dir = os.path.join(lc_dir, 'ucr_datasets_lc_Sub') 
	lc = np.load(os.path.join(lc_dir, args.dataset+'.npy'))
	...
	with open(out_path, 'w') as f:
		...
		# Train the model
		acc = train(X, y, train_idx, test_idx, distances, lc, device, logger, args.K, args.alpha, args.batch, args.epoch)
